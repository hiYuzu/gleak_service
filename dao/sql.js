module.exports = {
  ...
  selectMonitorDataByName:
    "select mtd.id,mtd.state,mtd.value,mt.name as monitorName ,us.name as userName,DATE_FORMAT(mtd.gmt_modified, '%Y-%m-%d %H:%i:%S') AS time from monitor_data as mtd inner join monitor as mt inner join user as us on mt.id=mtd.monitor_id and mtd.user_id=us.id where mt.name like ? limit ?,? order by mtd.gmt_modified desc",
